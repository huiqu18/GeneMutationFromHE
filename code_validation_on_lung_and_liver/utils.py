...
def obtain_test_results():
    dataset = 'TCGA-LIHC'
    data_type = 'pathway_cna'

    if dataset == 'TCGA-LUAD':
        if data_type == 'mutation':
            gene_or_pathway_names = ['ARID1A', 'KMT2C', 'NF1', 'NOTCH2', 'PIK3CA', 'RB1', 'RYR2', 'TP53', 'USH2A']
        elif data_type == 'cna':
            gene_or_pathway_names = ['APH1A', 'CDKN2A', 'CDKN2B', 'EIF4EBP1', 'FGFR1', 'HEY1', 'KAT6A', 'MCL1', 'MYC',
                                     'NCSTN', 'NOTCH2', 'PTK2', 'RAB25', 'RIT1']
        else:
            gene_or_pathway_names = ['pi3k', 'p53', 'notch', 'rtk', 'nrf2', 'wnt', 'tgfb', 'myc', 'cellcycle', 'hippo']
    else:
        if data_type == 'mutation':
            gene_or_pathway_names = ['ARID1A', 'KMT2C', 'NF1', 'RB1', 'RYR2', 'TP53', 'USH2A']
        elif data_type == 'cna':
            gene_or_pathway_names = ['AKT3', 'APH1A', 'CCND1', 'CDKN2A', 'CDKN2B', 'E2F5', 'EIF4EBP1', 'FGFR1', 'HEY1', 'IRF2BP2',
    ...
    for gene_or_pathway in sorted(gene_or_pathway_names):
        print(gene_or_pathway)
        file_path = './experiments/{:s}/{:s}/{:s}/test_best/test_results.txt'.format(dataset, data_type, gene_or_pathway)
        Acc, AUC, CIs = parse_test_results(file_path)
        txt_file.write('{:s}\t{:.2f}\t{:.2f} ({:.2f}, {:.2f})\n'.format(gene_or_pathway, Acc, AUC, CIs[0], CIs[1]))
...
def plot_roc_curve(save_filename, dataset, data_type):
    ...
    gene_or_pathway_names = ['CDH1', 'RB1', 'TP53', 'MAP3K1', 'TBX3', 'GATA3', 'NOTCH2']
    fig = plt.figure()
    ...
    for gene in gene_or_pathway_names:
