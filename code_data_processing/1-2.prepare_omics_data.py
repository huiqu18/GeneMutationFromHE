...
def extract_shared_patients_and_genes():
    # load data
    data_cna_selected = pd.read_pickle('{:s}/data_log2cna_step2.pickle'.format(omics_data_dir))
    data_expression_selected = pd.read_pickle('{:s}/data_expression_step2.pickle'.format(omics_data_dir))
    ...
    # save data
    data_cna.to_pickle('{:s}/data_cna_shared.pickle'.format(omics_data_dir))
    data_expression.to_pickle('{:s}/data_expression_shared.pickle'.format(omics_data_dir))
    np.save('{:s}/shared_patients.npy'.format(omics_data_dir), shared_patients)
    np.save('{:s}/shared_genes.npy'.format(omics_data_dir), shared_genes)
...
def get_cna(patient_names):
    ...
    data_cna.to_pickle('{:s}/data_log2cna_original.pickle'.format(omics_data_dir))

    data_cna_selected = data_cna.loc[data_cna['patient_name'].isin(patient_names)]
    data_cna_selected.to_pickle('{:s}/data_log2cna_step2.pickle'.format(omics_data_dir))
...
def get_gene_expression(patient_names):
    ...
    data_exp.to_pickle('{:s}/data_expression_original.pickle'.format(omics_data_dir))

    data_exp_selected = data_exp.loc[data_exp['patient_name'].isin(patient_names)]
    data_exp_selected.to_pickle('{:s}/data_expression_step2.pickle'.format(omics_data_dir))
